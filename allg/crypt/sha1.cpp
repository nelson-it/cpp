 ...
 *
 */
int Sha1::get(uint8_t *Message_Digest)
{
    int i;
    ...
    for (i = 0; i < SHA1HashSize; ++i)
        Message_Digest[i] = this->Intermediate_Hash[i >> 2] >> 8 * (3 - (i & 0x03));
 ...
 *
 */
int Sha1::set(const uint8_t *message_array, unsigned length)
{
    this->reset();
    ...
    {
        this->Message_Block[this->Message_Block_Index++] = (*message_array & 0xFF);
